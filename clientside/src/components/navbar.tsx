    ...
    return(
        <div>
            ...
            <div className={`flex flex-col justify-start items-start w-80 h-svh fixed top-0 pl-4 pt-5    gap-5 bg-blue-950 border-r-2 border-blue-800 border-opacity-20 z-10 ${show_nav_bar?'translate-x-0':'-translate-x-96'} transition-transform duration-500`}>
                ...
                <span className="fixed left-6 text-slate-200 text-xl font-semibold flex flex-row justify-center items-center gap-1" onClick={()=>ProfileHandler()} >
                    <img src={profileURL} className="w-9 rounded-full" />{username}
                </span>
               
                <span className="fixed left-72 text-slate-300  " onClick={()=>setShowNavBar(!show_nav_bar)}>
